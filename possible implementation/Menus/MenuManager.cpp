...
void MenuManager::clickPos(sf::Vector2i pos)
{
	...
	// switch from battlefield to selection menu
	if (_currentMenu == menuType::battlefield
		&& currentMenu == menuType::heroSelection)
	{
		heroSelectionMenu.resetSelectionMenuInformation();
		_currentMenu = currentMenu;
	}

	// switch from selection menu to main menu
	if (_currentMenu == menuType::heroSelection
